...
#         labels[0].after(500, l self.animate_images(labels, images, (index + 1) % len(images)))
...
def animate_images(labels, images, index):
    for i, label in enumerate(labels):
        label.config(image=images[(index - i) % len(images)])
    labels[0].after(500, lambda: animate_images(labels, images, (index + 1) % len(images)))
...
def show_splash():
    ...
    frame = tk.Frame(splash, width=450, height=450, bg='#E7F1FD')
    frame.place(x=0, y=0)
    ...
    animate_images(labels, images, 0)

    # Close the splash screen after a delay (adjust delay time as needed)
    splash.after(4000, close_splash, splash)

    splash.mainloop()

# Вызов функции show_splash
# show_splash()
